...
merge_XS_dem_bathy<-function(xs_lines, dem, samp.d, pts){
  ...
  if(!"z_bed" %in% colnames(pts@data)){
    stop("Points file needs \"z_bed\" column")
  } # close if
  
  plot(xs_lines, axes=TRUE)
  points(pts, col=pts$id, pch=20)
  ...
  for(index in 1:nrow(xs_lines)){
    ...
    bathy<-data.frame(x=temp.pts@coords[,1], y=temp.pts@coords[,2], station=gProject(temp.line,temp.pts), elev=temp.pts$z_bed, source="Bathy")
